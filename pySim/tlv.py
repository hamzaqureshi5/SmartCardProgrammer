...
class TlvMeta(abc.ABCMeta):
    ...
    def __new__(metacls, name, bases, namespace, **kwargs):
        ...
        if (
            nested is None
            or inspect.isclass(nested)
            and issubclass(nested, TLV_IE_Collection)
        ):
    ...
    def from_dict(self, decoded: dict):
        ...
        if self.nested_collection:
            ...
            if not expected_key_name in decoded:
                raise ValueError(
                    "Dict %s doesn't contain expected key %s"
    ...
    def from_tlv(self, do: bytes):
        ...
        if rawtag:
            if rawtag != self._compute_tag():
                raise ValueError(
                    "%s: Encountered tag %s doesn't match our supported tag %s"
        ...
        while len(remainder):
            ...
            if tag in self.members_by_tag:
                ...
                cls = type(
                    name,
                    (first.__base__,),
                    {"tag": tag, "possible_nested": [], "nested_collection_cls": None},
                )
        ...
        for i in decoded:
            for k in i.keys():
                if k in self.members_by_name:
                    ...
                    raise ValueError(
                        "%s: Unknown TLV Class %s in %s; expected %s"
