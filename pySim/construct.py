...
class Rpad(Adapter):
    ...
    def _encode(self, obj, context, path):
        ...
        if len(obj) > target_size:
            raise SizeofError(
                "Input ({}) exceeds target size ({})".format(len(obj), target_size)
            )
    ...
    def __bytes_required(self, i, minlen=0):
        if self.signed:
            raise NotImplementedError(
                "FIXME: Implement support for encoding signed integer"
            )
...
TonNpi = BitStruct(
    "ext" / Flag, "type_of_number" / TypeOfNumber, "numbering_plan_id" / NumberingPlan
)
