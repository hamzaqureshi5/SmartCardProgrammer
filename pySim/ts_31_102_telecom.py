    ...
    def _decode_bin(self, in_bin):
        ...
        for i in range(0, len(in_bin)):
            ...
            for bitno in range(0, 8):
                ...
                ret[service_nr] = {"activated": True if byte & (1 << bitno) else False}
            ...
            for s in sorted(files_by_service.keys()):
                ...
                for f in files_by_service[s]:
                    ...
                    if exists != should_exist:
                        ...
                        if exists:
                            cmd.perror(
                                "  ERROR: File %s is selectable but should not!" % f
                            )
                        else:
                            cmd.perror(
                                "  ERROR: File %s is not selectable (%s) but should!"
                                % (f, sw)
                            )
        finally:
...
class EF_PBR(LinFixedEF):
    ...
    def __init__(
        self, fid="4F30", name="EF.PBR", desc="Phone Book Reference", **kwargs
    ):
...
class EF_PSC(TransparentEF):
    ...
    def __init__(
        ...
        fid="4F22",
        name="EF.PSC",
        desc="Phone Book Synchronization Counter",
        **kwargs
    ):
        super().__init__(fid, name=name, desc=desc, **kwargs)
        # self._tlv = FIXME
...
class EF_PUID(TransparentEF):
    ...
    def __init__(
        self, fid="4F24", name="EF.PUID", desc="Previous Unique Identifer", **kwargs
    ):
        super().__init__(fid, name=name, desc=desc, **kwargs)
...
class EF_MML(BerTlvEF):
    def __init__(
        self, fid="4F47", name="EF.MML", desc="Multimedia Messages List", **kwargs
    ):
        super().__init__(fid, name=name, desc=desc, **kwargs)
...
class EF_MMDF(BerTlvEF):
    def __init__(
        self, fid="4F48", name="EF.MMDF", desc="Multimedia Messages Data File", **kwargs
    ):
        super().__init__(fid, name=name, desc=desc, **kwargs)
...
class EF_MCS_CONFIG(BerTlvEF):
    ...
    class McsConfigDataCollection(
        TLV_IE_Collection,
        nested=[
            McpttUeConfigurationData,
            McpttUserProfileData,
            McsGroupConfigurationData,
            McpttServiceConfigurationData,
            McsUeInitialConfigurationData,
            McdataUeConfigurationData,
            McdataUserProfileData,
            McdataServiceConfigurationData,
            McvideoUeConfigurationData,
            McvideoUserProfileData,
            McvideoServiceConfigurationData,
        ],
    ):
        pass
...
class EF_MST(EF_UServiceTable):
    def __init__(
        ...
        sfid=0x01,
        name="EF.MST",
        desc="MCS Service Table",
        size=(2, 2),
        table=EF_MST_map,
        **kwargs
    ):
        super().__init__(
            fid=fid, sfid=sfid, name=name, desc=desc, size=size, table=table
        )


class DF_MCS(CardDF):
    def __init__(
        self, fid="5F3D", name="DF.MCS", desc="Mission Critical Services", **kwargs
    ):
        super().__init__(fid=fid, name=name, desc=desc, **kwargs)
        files = [
            EF_MST(),
