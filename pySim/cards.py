...
class SimCard:
    ...
    def update_ad(self, mnc=None, opmode=None, ofm=None, path=EF["AD"]):
        ...
        # read from card
        raw_hex_data, sw = self._scc.read_binary(path, length=None, offset=0)
        ...
        if mnc and abstract_data["extensions"]:
            ...
            if mnclen > 3 or mnclen < 2:
                raise RuntimeError(
                    'invalid length of mnc "{}", expecting 2 or 3 digits'.format(mnc)
                )
...
class UsimCard(SimCard):
    ...
    def update_epdgid(self, epdgid):
        ...
        data, sw = self._scc.update_binary(EF_USIM_ADF_map["ePDGId"], epdgid_tlv)
        return sw
    ...
    def update_est(self, service, bit=1):
        ...
        if sw == "9000":
            content = enc_st(res, service, bit)
            (res, sw) = self._scc.update_binary(EF_USIM_ADF_map["EST"], content)
        return sw
    ...
    def read_impu(self):
        ...
        for i in range(0, rec_cnt):
            ...
            if sw == "9000":
                ...
                impu_recs += (
                    "IMS public user identity: Can't read, response code = %s\n" % (sw)
                )
        return impu_recs
...
class MagicSimBase(abc.ABC, SimCard):
    ...
    * 3f00/7f4d/8f0e : Programmable Record EFs
    ...
    def program(self, p):
        ...
        self._scc.update_record(
            self._files["b_ef"][0], 1, rpad(v, self._files["b_ef"][1] * 2)
        )
...
class FakeMagicSim(SimCard):
    ...
    def program(self, p):
        ...
        entry = (
            "81"
            + rpad(s2h(p["name"][0:14]), 28)  # 1b  Status: Valid & Active
            + enc_iccid(p["iccid"])  # 14b  Entry Name
            + enc_imsi(p["imsi"])  # 10b  ICCID
            + p["ki"]  # 9b  IMSI_len + id_type(9) + IMSI
            + lpad(p["smsp"], 80)  # 16b  Ki  # 40b  SMSP (padded with ff if needed)
        )
...
class SysmoUSIMgr1(UsimCard):
    ...
    def program(self, p):
        ...
        par = (
            p["ki"]
            + p["opc"]  # 16b  K
            + enc_iccid(p["iccid"])  # 32b  OPC
            + enc_imsi(p["imsi"])  # 10b  ICCID  # 9b  IMSI_len + id_type(9) + IMSI
        )
...
class FairwavesSIM(UsimCard):
    ...
    def autodetect(kls, scc):
        try:
            # Look for ATR
            if scc.get_atr() == toBytes(
                "3B 9F 96 80 1F C7 80 31 A0 73 BE 21 13 67 44 22 06 10 00 00 01 A9"
            ):
                return kls(scc)
        except:
            return None
...
class WavemobileSim(UsimCard):
    ...
    def autodetect(kls, scc):
        try:
            # Look for ATR
            if scc.get_atr() == toBytes(
                "3B 9F 95 80 1F C7 80 31 E0 73 F6 21 13 67 4D 45 16 00 43 01 00 8F"
            ):
                return kls(scc)
        except:
            return None
        ...
        if self.adf_present("usim"):
            ...
            if (p.get("mcc") and p.get("mnc")) or p.get("opmode"):
                if p.get("mcc") and p.get("mnc"):
                    ...
                    mnc = None
            sw = self.update_ad(
                mnc=mnc, opmode=p.get("opmode"), path=EF_USIM_ADF_map["AD"]
            )
            if sw != "9000":
                print("Programming AD failed with code %s" % sw)
...
class GialerSim(UsimCard):
    ...
    def autodetect(cls, scc):
        try:
            # Look for ATR
            if scc.get_atr() == toBytes(
                "3B 9F 95 80 1F C7 80 31 A0 73 B6 A1 00 67 CF 32 15 CA 9C D7 09 20"
            ):
                return cls(scc)
        except:
            return None
