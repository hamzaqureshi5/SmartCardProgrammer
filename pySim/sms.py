...
def smpp_dcs_is_8bit(dcs: pdu_types.DataCoding) -> bool:
    ...
    if dcs == pdu_types.DataCoding(
        pdu_types.DataCodingScheme.DEFAULT,
    ...
    if (
        dcs.scheme == pdu_types.DataCodingScheme.GSM_MESSAGE_CLASS
        and dcs.schemeData["msgCoding"] == pdu_types.DataCodingGsmMsgCoding.DATA_8BIT
    ):
        return True
    else:
        return False
...
class AddressField:
    ...
    def fromSmpp(cls, addr, ton, npi) -> "AddressField":
        ...
        return cls(
            addr.decode("ascii"),
            AddressField.smpp_map_ton[ton.name],
            AddressField.smpp_map_npi[npi.name],
        )
    ...
    def toSmpp(self):
        ...
        return (
            self.digits,
            self.smpp_map_ton.inverse[self.ton],
            self.smpp_map_npi.inverse[self.npi],
        )
...
class SMS_DELIVER(SMS_TPDU):
    ...
    def __repr__(self):
        return (
            "%s(MTI=%s, MMS=%s, LP=%s, RP=%s, UDHI=%s, SRI=%s, OA=%s, PID=%2x, DCS=%x, SCTS=%s, UDL=%u, UD=%s)"
            % (
    ...
    def fromSmppSubmit(cls, smpp_pdu) -> "SMS_DELIVER":
        ...
        tp_oa = AddressField.fromSmpp(
            smpp_pdu.params["source_addr"],
        ...
        d = {
            ...
            "tp_sri": False,
            "tp_rp": False,
            "tp_udhi": pdu_types.EsmClassGsmFeatures.UDHI_INDICATOR_SET
            in smpp_pdu.params["esm_class"].gsmFeatures,
...
class SMS_SUBMIT(SMS_TPDU):
    ...
    def __repr__(self):
        return (
            "%s(MTI=%s, RD=%s, VPF=%u, RP=%s, UDHI=%s, SRR=%s, DA=%s, PID=%2x, DCS=%x, VP=%s, UDL=%u, UD=%s)"
            % (
                self.__class__.__name__,
                self.tp_mti,
                self.tp_rd,
    ...
    def fromSmppSubmit(cls, smpp_pdu) -> "SMS_SUBMIT":
        ...
        ensure_smpp_is_8bit(smpp_pdu.params["data_coding"])
        tp_da = AddressField.fromSmpp(
            smpp_pdu.params["destination_addr"],
        ...
        d = {
            "tp_rd": True
            if smpp_pdu.params["replace_if_present_flag"].name == "REPLACE"
            else False,
            "tp_vpf": None,  # vpf,
    ...
    def toSmpp(self) -> pdu_types.PDU:
        ...
        esm_class = pdu_types.EsmClass(
            pdu_types.EsmClassMode.DEFAULT, pdu_types.EsmClassType.DEFAULT
        )
        reg_del = pdu_types.RegisteredDelivery(
