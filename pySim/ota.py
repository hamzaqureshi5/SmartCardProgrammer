...
CompactRemoteResp = Struct(
    "number_of_commands" / Int8ub,
...
SPI = BitStruct(  # first octet
    ...
    / Enum(
        BitsInteger(2),
        no_counter=0,
        counter_no_replay_or_seq=1,
        counter_must_be_higher=2,
        counter_must_be_lower=3,
    ),
    "ciphering" / Flag,
    "rc_cc_ds" / RC_CC_DS,
    # second octet
    ...
    "por" / Enum(BitsInteger(2), no_por=0, por_required=1, por_only_when_error=2),
)
...
class OtaDialect(abc.ABC):
    ...
    @abc.abstractmethod
    def decode_resp(
        self, otak: OtaKeyset, apdu: bytes
    ) -> (object, Optional["CompactRemoteResp"]):
...
class OtaAlgoCrypt(OtaAlgo, abc.ABC):
    def __init__(self, otak: OtaKeyset):
        if self.enum_name != otak.algo_crypt:
            raise ValueError(
                "Cannot use algorithm %s with key for %s"
...
class OtaAlgoAuth(OtaAlgo, abc.ABC):
    ...
    def check_sig(self, data: bytes, cc_received: bytes):
        ...
        if cc_received != cc:
            raise OtaCheckError(
                "Received CC (%s) != Computed CC (%s)" % (b2h(cc_received), b2h(cc))
            )
...
class OtaDialectSms(OtaDialect):
    ...
    def encode_cmd(self, otak: OtaKeyset, tar: bytes, spi: dict, apdu: bytes) -> bytes:
        ...
        # CHL + SPI (+ KIC + KID)
        c = Struct(
            "chl" / Int8ub, "spi" / SPI, "kic" / KIC, "kid" / KID_CC, "tar" / Bytes(3)
        )
        part_head = c.build(
            {"chl": chl, "spi": spi, "kic": kic, "kid": kid, "tar": tar}
        )
    ...
    def decode_resp(
        self, otak: OtaKeyset, spi: dict, data: bytes
