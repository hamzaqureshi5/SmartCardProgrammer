        ...
        if service in range((8 * i) + 1, (8 * i) + 9):
            ...
            for j in range(1, 9):
                ...
                if service == (8 * i) + j:
                    ...
                    mod_byte = (
                        byte & 0x01 == 0x01 and mod_byte | 0x80 or mod_byte & 0x7F
                    )
                byte = byte >> 1
...
def dec_ePDGSelection(sixhexbytes):
    ...
    epdg_fqdn_format_str = sixhexbytes[
        plmn_chars
        + epdg_priority_chars : plmn_chars
        + epdg_priority_chars
        + epdg_fqdn_format_chars
    ]
    res["mcc"] = dec_mcc_from_plmn(plmn_str)
    res["mnc"] = dec_mnc_from_plmn(plmn_str)
    res["epdg_priority"] = epdg_priority_str
    res["epdg_fqdn_format"] = (
        epdg_fqdn_format_str == "00"
        and "Operator Identifier FQDN"
        or "Location based FQDN"
    )
    return res
...
def format_ePDGSelection(hexstr):
    ...
    len_str = hexstr[
        ePDGSelection_info_tag_chars : ePDGSelection_info_tag_chars + len_chars
    ]
    ...
    for rec_data in hexstr_to_Nbytearr(content_str, 6):
        ...
        if rec_info["mcc"] == 0xFFF and rec_info["mnc"] == 0xFFF:
            ...
            rec_str = "MCC: %03d MNC: %03d ePDG Priority: %s ePDG FQDN format: %s" % (
                rec_info["mcc"],
                rec_info["mnc"],
                rec_info["epdg_priority"],
                rec_info["epdg_fqdn_format"],
            )
        s += "\t%s # %s\n" % (rec_data, rec_str)
    return s
...
def tabulate_str_list(
    str_list, width: int = 79, hspace: int = 2, lspace: int = 1, align_left: bool = True
) -> str:
...
class DataObject(abc.ABC):
    ...
    def __init__(
        self, name: str, desc: Optional[str] = None, tag: Optional[int] = None
    ):
        """
        Args:
    ...
    def to_tlv(self) -> bytes:
        ...
        val = self.to_bytes()
        return (
            bertlv_encode_tag(self._compute_tag()) + bertlv_encode_len(len(val)) + val
        )
    ...
    def decode(self, binary: bytes) -> Tuple[dict, bytes]:
        ...
        if tag != self.tag:
            raise ValueError(
                "%s: Unknown Tag 0x%02x in %s; expected 0x%02x"
    ...
    def decode(self, binary: bytes) -> Tuple[List, bytes]:
        ...
        while len(remainder):
            ...
            if not tag in self.members_by_tag:
                raise ValueError(
                    "%s: Unknown Tag 0x%02x in %s; expected %s"
...
class DataObjectSequence:
    ...
    def __add__(self, other) -> "DataObjectSequence":
        ...
        if isinstance(other, "DataObject"):
            ...
            return DataObjectSequence(
                self.name, self.desc, self.sequence + other.sequence
            )
