...
class EF_AD(TransparentEF):
    ...
    _test_de_encode = [
        (
            "000000",
            {"ms_operation_mode": "normal", "additional_info": "0000", "rfu": ""},
        ),
    ]
...
class EF_SMS(LinFixedEF):
    ...
    def __init__(
        ...
        super().__init__(
            fid, sfid=sfid, name=name, desc=desc, rec_len=(2, 255), **kwargs
        )
        self._construct = Struct(
            # Byte 1: Status
            "status"
            / BitStruct(
                ...
                "rfu54" / BitsRFU(2),
                "status"
                / FlagsEnum(BitsInteger(2), read=0, to_be_read=1, sent=2, to_be_sent=3),
                "used" / Flag,
            ),
            # Byte 2: Length
            "length" / Int8ub,
...
class DF_CDMA(CardDF):
    def __init__(self):
        super().__init__(
            fid="7f25", name="DF.CDMA", desc="CDMA related files (3GPP2 C.S0023-D)"
        )
        files = [
...
class CardProfileRUIM(CardProfile):
    ...
    def __init__(self):
        super().__init__(
            "R-UIM",
            desc="CDMA R-UIM Card",
            cla="a0",
            sel_ctrl="0000",
            files_in_mf=[DF_TELECOM(), DF_GSM(), DF_CDMA()],
        )
