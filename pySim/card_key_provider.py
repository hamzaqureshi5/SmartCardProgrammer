...
class CardKeyProvider(abc.ABC):
    ...
    def _verify_get_data(
        self, fields: List[str] = [], key: str = "ICCID", value: str = ""
    ) -> Dict[str, str]:
        ...
        for f in fields:
            if f not in self.VALID_FIELD_NAMES:
                raise ValueError(
                    "Requested field name '%s' is not a valid field name, valid field names are: %s"
                    % (f, str(self.VALID_FIELD_NAMES))
                )

        if key not in self.VALID_FIELD_NAMES:
            raise ValueError(
                "Key field name '%s' is not a valid field name, valid field names are: %s"
    ...
    def get_field(
        self, field: str, key: str = "ICCID", value: str = ""
    ) -> Optional[str]:
            ...
            if row[key] == value:
                for f in fields:
                    if f in row:
                        ...
                        raise RuntimeError(
                            "CSV-File '%s' lacks column '%s'" % (self.filename, f)
                        )
        return rc


def card_key_provider_register(
    provider: CardKeyProvider, provider_list=card_key_providers
):
...
def card_key_provider_get(
    fields, key: str, value: str, provider_list=card_key_providers
) -> Dict[str, str]:
...
def card_key_provider_get_field(
    field: str, key: str, value: str, provider_list=card_key_providers
) -> Optional[str]:
