...
class Address(COMPR_TLV_IE, tag=0x06):
    _construct = Struct(
        "ton_npi" / Int8ub, "call_number" / BcdAdapter(Bytes(this._.total_len - 1))
    )
...
class DeviceIdentities(COMPR_TLV_IE, tag=0x82):
    ...
    def _from_bytes(self, do: bytes):
        return {
            "source_dev_id": self.DEV_IDS[do[0]],
            "dest_dev_id": self.DEV_IDS[do[1]],
        }
...
class Duration(COMPR_TLV_IE, tag=0x04):
    _construct = Struct(
        "time_unit" / Enum(Int8ub, minutes=0, seconds=1, tenths_of_seconds=2),
...
class Item(COMPR_TLV_IE, tag=0x0F):
    _construct = Struct(
        "identifier" / Int8ub, "text_string" / GsmStringAdapter(GreedyBytes)
    )
...
class ResponseLength(COMPR_TLV_IE, tag=0x11):
    _construct = Struct("minimum_length" / Int8ub, "maximum_length" / Int8ub)
...
class TextString(COMPR_TLV_IE, tag=0x0D):
    _construct = Struct(
        "dcs" / Int8ub, "text_string" / HexAdapter(GreedyBytes)  # TS 03.38
    )
...
class USSDString(COMPR_TLV_IE, tag=0x0A):
    _construct = Struct("dcs" / Int8ub, "ussd_string" / HexAdapter(GreedyBytes))


# TS 102 223 Section 8.18
class FileList(COMPR_TLV_IE, tag=0x12):
    FileId = HexAdapter(Bytes(2))
    _construct = Struct("number_of_files" / Int8ub, "files" / GreedyRange(FileId))
...
class DefaultText(COMPR_TLV_IE, tag=0x97):
    _construct = Struct("dcs" / Int8ub, "text_string" / HexAdapter(GreedyBytes))
...
class IconIdentifier(COMPR_TLV_IE, tag=0x1E):
    _construct = Struct(
        "icon_qualifier" / FlagsEnum(Int8ub, not_self_explanatory=1),
...
class ItemIconIdentifierList(COMPR_TLV_IE, tag=0x9F):
    _construct = Struct(
        "icon_list_qualifier" / FlagsEnum(Int8ub, not_self_explanatory=1),
...
class TimingAdvance(COMPR_TLV_IE, tag=0x46):
    _construct = Struct(
        "me_status" / Enum(Int8ub, in_idle_state=0, not_in_idle_state=1),
...
class BearerDescription(COMPR_TLV_IE, tag=0xB5):
    ...
    BearerParsCs = Struct(
        "data_rate" / Int8ub, "bearer_service" / Int8ub, "connection_element" / Int8ub
    )
    # TS 31.111 Section 8.52.2
    BearerParsPacket = Struct(
        "precendence_class" / Int8ub,
        "delay" / Int8ub,
        "reliability" / Int8ub,
    ...
    BearerParsPacketExt = Struct(
        "traffic_class" / Int8ub,
    ...
    _construct = Struct(
        ...
        / Enum(
            ...
            csd=1,
            packet_grps_utran_eutran=2,
            packet_with_extd_params=9,
            wlan=0x0A,
            packet_eutran_mapped_utran=0x0B,
            ng_ran=0x0C,
            # TS 102 223 Section 8.52
            default=3,
            local_link=4,
...
class OtherAddress(COMPR_TLV_IE, tag=0xBE):
    _construct = Struct(
        "type_of_address" / Enum(Int8ub, ipv4=0x21, ipv6=0x57),
...
class UiccTransportLevel(COMPR_TLV_IE, tag=0xBC):
    _construct = Struct(
        ...
        / Enum(
            Int8ub,
            udp_uicc_client_remote=1,
            tcp_uicc_client_remote=2,
            tcp_uicc_server=3,
            udp_uicc_client_local=4,
            tcp_uicc_client_local=5,
            direct_channel=6,
        ),
        "port_number" / Int16ub,
    )
...
class ServiceRecord(COMPR_TLV_IE, tag=0xC1):
    BearerTechId = Enum(
        Int8ub, technology_independent=0, bluetooth=1, irda=2, rs232=3, usb=4
    )
    _construct = Struct(
        "local_bearer_technology" / BearerTechId,
...
class DeviceFilter(COMPR_TLV_IE, tag=0xC2):
    _construct = Struct(
        "local_bearer_technology" / ServiceRecord.BearerTechId,
...
class RemoteEntityAddress(COMPR_TLV_IE, tag=0xC9):
    _construct = Struct(
        "coding_type" / Enum(Int8ub, ieee802_16=0, irda=1),
        "address" / HexAdapter(GreedyBytes),
    )
...
class FrameLayout(COMPR_TLV_IE, tag=0xE6):
    _construct = Struct(
        "layout" / Enum(Int8ub, horizontal=1, vertical=2),
...
class RoutingAreaIdentification(COMPR_TLV_IE, tag=0xF3):
    _construct = Struct(
        "mcc_mnc" / BcdAdapter(Bytes(3)), "lac" / HexAdapter(Bytes(2)), "rac" / Int8ub
    )
...
class UpdateAttachRegistrationType(COMPR_TLV_IE, tag=0xF4):
    _construct = Enum(
        Int8ub,
        normal_location_updating_lu=0x00,
...
class EcatSequenceNumber(COMPR_TLV_IE, tag=0xA1):
    ...
    _construct = BitStruct(
        "command_type_indicator" / CmdTypeIndicator, "counter" / BitsInteger(22)
    )
...
class RefreshEnforcementPolicy(COMPR_TLV_IE, tag=0x3A):
    _construct = FlagsEnum(
        Byte, even_if_navigating_menus=0, even_if_data_call=1, even_if_voice_call=2
    )
...
class SupportedRadioAccessTechnologies(COMPR_TLV_IE, tag=0xB4):
    AccessTechTuple = Struct(
        "technology" / AccessTechnology.SingleAccessTech,
...
class SMSPPDownload(BER_TLV_IE, tag=0xD1, nested=[DeviceIdentities, Address, SMS_TPDU]):
    pass


# TS 101 220 Table 7.17 + 31.111 7.1.1.3
class SMSCBDownload(BER_TLV_IE, tag=0xD2, nested=[DeviceIdentities, CBSPage]):
    pass


class USSDDownload(BER_TLV_IE, tag=0xD9, nested=[DeviceIdentities, USSDString]):
    pass
...
class Refresh(
    ...
    nested=[
        ...
        ApplicationSpecificRefreshData,
        PlmnWactList,
        PlmnList,
    ],
):
...
class MoreTime(ProactiveCmd, tag=0x02, nested=[CommandDetails]):
    pass


class PollInterval(ProactiveCmd, tag=0x03, nested=[CommandDetails]):
    pass


class PollingOff(ProactiveCmd, tag=0x04, nested=[CommandDetails]):
    pass


class SetUpEventList(ProactiveCmd, tag=0x05, nested=[CommandDetails]):
    pass
...
class SendDTMF(ProactiveCmd, tag=0x14, nested=[CommandDetails]):
    pass


class LaunchBrowser(ProactiveCmd, tag=0x15, nested=[CommandDetails]):
    pass


class GeographicalLocationRequest(ProactiveCmd, tag=0x16, nested=[CommandDetails]):
    pass


class PlayTone(ProactiveCmd, tag=0x20, nested=[CommandDetails]):
    pass
...
class SelectItem(
    ...
    nested=[
        ...
        DeviceIdentities,
        AlphaIdentifier,  #
        ItemsNextActionIndicator,
        ItemIdentifier,
        IconIdentifier,
        ItemIconIdentifierList,
        TextAttribute,
        ItemTextAttributeList,
        FrameIdentifier,
    ],
):
    pass
...
# TS 102 223 6.6.15
class ProvideLocalInformation(
    ProactiveCmd, tag=0x26, nested=[CommandDetails, DeviceIdentities]
):
    pass
...
# TS 102 223 6.6.19
class PowerOnCard(ProactiveCmd, tag=0x31, nested=[CommandDetails, DeviceIdentities]):
    pass


# TS 102 223 6.6.18
class PowerOffCard(ProactiveCmd, tag=0x32, nested=[CommandDetails, DeviceIdentities]):
    pass


# TS 102 223 6.6.20
class GetReaderStatus(
    ProactiveCmd, tag=0x33, nested=[CommandDetails, DeviceIdentities]
):
    pass
...
class LanguageNotification(
    ProactiveCmd, tag=0x35, nested=[CommandDetails, DeviceIdentities, Language]
):
    pass
...
# TS 102 223 6.6.31
class GetChannelStatus(
    ProactiveCmd, tag=0x44, nested=[CommandDetails, DeviceIdentities]
):
    pass
...
# TS 102 223 6.6.36
class GetFramesStatus(
    ProactiveCmd, tag=0x51, nested=[CommandDetails, DeviceIdentities]
):
    pass


# TS 102 223 6.6.37
class RetrieveMultimediaMessage(
    ProactiveCmd,
    tag=0x60,
    nested=[
        CommandDetails,
        DeviceIdentities,
        AlphaIdentifier,
        MultimediaMessageReference,
        FileList,
        MmContentIdentifier,
        MultimediaMessageIdentifier,
        TextAttribute,
        FrameIdentifier,
    ],
):
...
# TS 102 223 6.6.39
class DisplayMultimediaMessage(
    ProactiveCmd,
    tag=0x62,
    nested=[
        CommandDetails,
...
# TS 102 223 6.6.43
class EncapsulatedSessionControl(
    ProactiveCmd, tag=0x73, nested=[CommandDetails, DeviceIdentities, SaTemplate]
):
    pass
