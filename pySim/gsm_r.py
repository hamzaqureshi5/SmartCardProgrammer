...
class EF_FN(LinFixedEF):
    ...
    def __init__(self):
        super().__init__(
            ...
            sfid=None,
            name="EF.FN",
            desc="Functional numbers",
            rec_len=(9, 9),
        )
        self._construct = Struct(
            "functional_number_and_type" / FuncNTypeAdapter(Bytes(8)),
            "list_number" / Int8ub,
        )
...
NextTableType = Enum(
    Byte, decision=0xF0, predefined=0xF1, num_dial_digits=0xF2, ic=0xF3, empty=0xFF
)
...
class EF_CallconfI(LinFixedEF):
    ...
    def __init__(self):
        ...
        self._construct = Struct(
            ...
            "cause"
            / FlagsEnum(Int8ub, powered_off=1, radio_link_error=2, user_command=5),
...
class EF_Shunting(TransparentEF):
    ...
    def __init__(self):
        super().__init__(
            fid="6ff4", sfid=None, name="EF.Shunting", desc="Shunting", size=(8, 8)
        )
        self._construct = Struct(
            "common_gid" / Int8ub, "shunting_gid" / HexAdapter(Bytes(7))
        )
...
class EF_GsmrPLMN(LinFixedEF):
    ...
    def __init__(self):
        super().__init__(
            ...
            sfid=None,
            name="EF.GsmrPLMN",
            desc="GSM-R network selection",
            rec_len=(9, 9),
        )
        self._construct = Struct(
            "plmn" / BcdAdapter(Bytes(3)),
            "class_of_network"
            / BitStruct(
                "supported"
                / FlagsEnum(BitsInteger(5), vbs=1, vgcs=2, emlpp=4, fn=8, eirene=16),
...
class EF_IC(LinFixedEF):
    ...
    def __init__(self):
        super().__init__(
            ...
            sfid=None,
            name="EF.IC",
            desc="International Code",
            rec_len=(7, 7),
        )
        self._construct = Struct(
            "next_table_type" / NextTableType,
...
class EF_NW(LinFixedEF):
    ...
    def __init__(self):
        super().__init__(
            fid="6f80", sfid=None, name="EF.NW", desc="Network Name", rec_len=(8, 8)
        )
...
class EF_Predefined(LinFixedEF):
    ...
    _test_de_encode = [
        (
            "f26f85",
            1,
            {"next_table_type": "num_dial_digits", "id_of_next_table": "6f85"},
        ),
        ("f0ffc8", 2, {"predefined_value1": "0fff", "string_table_index1": 200}),
    ]
    # header and other records have different structure. WTF !?!
    construct_first = Struct(
        "next_table_type" / NextTableType, "id_of_next_table" / HexAdapter(Bytes(2))
    )
    construct_others = Struct(
        "predefined_value1" / BcdAdapter(Bytes(2)), "string_table_index1" / Int8ub
    )
...
class DF_EIRENE(CardDF):
    def __init__(self, fid="7fe0", name="DF.EIRENE", desc="GSM-R EIRENE"):
        ...
        files = [
            ...
            EF_Predefined(
                fid="6f8a", name="EF.Call", desc="First digit of the group ID"
            ),
            EF_Predefined(
                fid="6f8b",
                name="EF.FctTeam",
            ...
            EF_Predefined(
                fid="6f8c", name="EF.Gateway", desc="Access to external networks"
            ),
            EF_DialledVals(
                fid="6f81",
                name="EF.5to8digits",
            ...
            EF_DialledVals(
                fid="6f86", name="EF.LLLLL", desc="Location number Call Type 6"
            ),
            EF_DialledVals(
                fid="6f91", name="EF.Location", desc="Location number Call Type 7"
            ),
            EF_DialledVals(
                fid="6f87", name="EF.FreeNumber", desc="Free Number Call Type 0 and 8"
            ),
        ]
