...
class UsimAuthenticateEven(
    ApduCommand, n="AUTHENTICATE", ins=0x88, cla=["0X", "4X", "6X"]
):
    _apdu_case = 4
    _construct_p2 = BitStruct(
        ...
        / Enum(BitsInteger(3), gsm=0, umts=1, vgcs_vbs=2, gba=4),
    )
    _cs_cmd_gsm_3g = Struct(
        "_rand_len" / Int8ub,
    ...
    _cs_cmd_gba = Struct(
        ...
        / Switch(
            this.tag,
            {0xDD: "bootstrap" / _cmd_gba_bs, 0xDE: "naf_derivation" / _cmd_gba_naf},
        ),
    )
    _cs_rsp_gsm = Struct(
        "_len_sres" / Int8ub,
    ...
    _rsp_3g_ok = Struct(
        ...
        "_len_ck" / Int8ub,
        "ck" / HexAdapter(Bytes(this._len_ck)),
        "_len_ik" / Int8ub,
        "ik" / HexAdapter(Bytes(this._len_ik)),
        "_len_kc" / COptional(Int8ub),
        "kc" / If(this._len_kc, HexAdapter(Bytes(this._len_kc))),
    )
    _rsp_3g_sync = Struct(
        "_len_auts" / Int8ub, "auts" / HexAdapter(Bytes(this._len_auts))
    )
    _cs_rsp_3g = Struct(
        "tag" / Int8ub,
        "body"
        / Switch(
            this.tag, {0xDB: "success" / _rsp_3g_ok, 0xDC: "sync_fail" / _rsp_3g_sync}
        ),
    )
    _cs_rsp_vgcs = Struct(
        Const(b"\xDB"), "_vstk_len" / Int8ub, "vstk" / HexAdapter(Bytes(this._vstk_len))
    )
    _cs_rsp_gba_naf = Struct(
        Const(b"\xDB"),
        "_ks_ext_naf_len" / Int8ub,
...
class UsimAuthenticateOdd(
    ApduCommand, n="AUTHENTICATE", ins=0x89, cla=["0X", "4X", "6X"]
):
    _apdu_case = 4
    _construct_p2 = BitStruct(
        "scope" / Enum(Flag, mf=0, df_adf_specific=1),
...
ApduCommands = ApduCommandSet(
    "TS 31.102", cmds=[UsimAuthenticateEven, UsimAuthenticateOdd, UsimGetIdentity]
)
