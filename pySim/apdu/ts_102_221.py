...
class UiccSelect(ApduCommand, n="SELECT", ins=0xA4, cla=["0X", "4X", "6X"]):
    ...
    _construct_p1 = Enum(
        Byte,
        df_ef_or_mf_by_file_id=0,
        child_df_of_current_df=1,
        parent_df_of_current_df=3,
        df_name=4,
        path_from_mf=8,
        path_from_current_df=9,
    )
    _construct_p2 = BitStruct(
        Flag,
        "app_session_control" / Enum(BitsInteger(2), activation_reset=0, termination=2),
        "return" / Enum(BitsInteger(3), fcp=1, no_data=3),
        "aid_control"
        / Enum(BitsInteger(2), first_or_only=0, last=1, next=2, previous=3),
    )
            ...
            for file in path:
                ...
                if file_hex == "7fff":  # current application
                    if not lchan.selected_adf:
                        ...
                        # HACK: Assume USIM
                        logger.warning(
                            "SELECT relative to current ADF, but no ADF selected. Assuming ADF.USIM"
                        )
                    ...
                    sels = lchan.selected_file.get_selectables(
                        ["FIDS", "MF", "PARENT", "SELF"]
                    )
                    if file_hex in sels:
                        if self.successful:
                        ...
                        continue
                logger.warning(
                    "SELECT UNKNOWN FID %s (%s)"
                    % (file_hex, "/".join([b2h(x) for x in path]))
                )
...
class UiccStatus(ApduCommand, n="STATUS", ins=0xF2, cla=["8X", "CX", "EX"]):
    ...
    _construct_p1 = Enum(
        Byte,
        no_indication=0,
        current_app_is_initialized=1,
        terminal_will_terminate_current_app=2,
    )
    _construct_p2 = Enum(
        Byte, response_like_select=0, response_df_name_tlv=1, response_no_data=0x0C
    )
...
PinConstructP2 = BitStruct(
    "scope" / Enum(Flag, global_mf=0, specific_df_adf=1),
    BitsInteger(2),
...
class DeactivateFile(
    ApduCommand, n="DEACTIVATE FILE", ins=0x04, cla=["0X", "4X", "6X"]
):
    _apdu_case = 1
    _construct_p1 = BitStruct(
        BitsInteger(4),
        "select_mode"
        / Enum(BitsInteger(4), ef_by_file_id=0, path_from_mf=8, path_from_current_df=9),
    )
...
auth_p2_construct = BitStruct(
    "scope" / Enum(Flag, mf=0, df_adf_specific=1),
    BitsInteger(2),
    "reference_data_nr" / BitsInteger(5),
)
...
class TerminalCapability(
    ApduCommand, n="TERMINAL CAPABILITY", ins=0xAA, cla=["8X", "CX", "EX"]
):
    _apdu_case = 3


# TS 102 221 Section 11.1.20
class ManageSecureChannel(
    ApduCommand, n="MANAGE SECURE CHANNEL", ins=0x73, cla=["0X", "4X", "6X"]
):
    @classmethod
    def _get_apdu_case(cls, hdr: bytes) -> int:
        ...
        if p1 & 0x7 == 0:  # retrieve UICC Endpoints
            ...
            return 3
        raise ValueError(
            "%s: Unable to detect APDU case for %s" % (cls.__name__, b2h(hdr))
        )
...
class SuspendUicc(ApduCommand, n="SUSPEND UICC", ins=0x76, cla=["80"]):
    _apdu_case = 4
    _construct_p1 = BitStruct(
        "rfu" / BitsInteger(7), "mode" / Enum(Flag, suspend=0, resume=1)
    )
...
class GetIdentity(ApduCommand, n="GET IDENTITY", ins=0x78, cla=["8X", "CX", "EX"]):
    _apdu_case = 4
    _construct_p2 = BitStruct(
        "scope" / Enum(Flag, mf=0, df_adf_specific=1), BitsInteger(7)
    )


# TS 102 221 Section 11.1.24
class ExchangeCapabilities(
    ApduCommand, n="EXCHANGE CAPABILITIES", ins=0x7A, cla=["80"]
):
    _apdu_case = 4
