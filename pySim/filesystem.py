...
class CardFile:
    ...
    def _get_self_selectables(
        self, alias: str = None, flags=[]
    ) -> Dict[str, "CardFile"]:
    ...
    def _get_parent_selectables(
        self, alias: Optional[str] = None, flags=[]
    ) -> Dict[str, "CardFile"]:
        sels = {}
                ...
                if isinstance(c, CardDF):
                    for gc in c.children.values():
                        if isinstance(gc, CardDF):
                            if gc._has_service():
                                raise ValueError(
                                    "TODO: implement recursive service -> file mapping"
                                )
...
class CardEF(CardFile):
    ...
    def get_selectables(self, flags=[]) -> dict:
        ...
        if flags == [] or "FIDS" in flags:
            sels.update(
                {x.fid: x for x in self.parent.children.values() if x.fid and x != self}
            )
        if flags == [] or "FNAMES" in flags:
            sels.update(
                {
                    x.name: x
                    for x in self.parent.children.values()
                    if x.name and x != self
                }
            )
        return sels
...
class TransparentEF(CardEF):
    ...
    class ShellCommands(CommandSet):
        ...
        dec_hex_parser.add_argument(
            "--oneline",
            action="store_true",
            help="No JSON pretty-printing, dump as a single line",
        )
        dec_hex_parser.add_argument(
            "HEXSTR", help="Hex-string of encoded data to decode"
        )
        ...
        read_bin_dec_parser.add_argument(
            "--oneline",
            action="store_true",
            help="No JSON pretty-printing, dump as a single line",
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(
            fid=fid, sfid=sfid, name=name, desc=desc, parent=parent, **kwargs
        )
...
class LinFixedEF(CardEF):
    ...
    class ShellCommands(CommandSet):
        ...
        read_rec_parser.add_argument(
            ...
            type=int,
            default=1,
            help="Number of records to be read, beginning at record_nr",
        )
        ...
        read_recs_dec_parser.add_argument(
            "--oneline",
            action="store_true",
            help="No JSON pretty-printing, dump as a single line",
        )
        ...
        def do_update_record_decoded(self, opts):
            ...
            if opts.json_path:
                ...
                data_json = json.loads(opts.data)
            (data, sw) = self._cmd.lchan.update_record_dec(opts.record_nr, data_json)
            if data:
                self._cmd.poutput(data)
...
class TransRecEF(TransparentEF):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(
            fid=fid, sfid=sfid, name=name, desc=desc, parent=parent, size=size, **kwargs
        )
        self.rec_len = rec_len
...
class RuntimeLchan:
    ...
    def probe_file(self, fid: str, cmd_app=None):
        ...
        if not is_hex(fid, 4, 4):
            raise ValueError(
                "Cannot select unknown file by name %s, only hexadecimal 4 digit FID is allowed"
                % fid
            )
        ...
        if select_resp["file_descriptor"]["file_descriptor_byte"]["file_type"] == "df":
            f = CardDF(
                fid=fid,
                sfid=None,
                name="DF." + str(fid).upper(),
                desc="dedicated file, manually added at runtime",
            )
        else:
            if (
                select_resp["file_descriptor"]["file_descriptor_byte"]["structure"]
                == "transparent"
            ):
                f = TransparentEF(
                    fid=fid,
                    sfid=None,
                    name="EF." + str(fid).upper(),
    ...
    def select_file(self, file: CardFile, cmd_app=None):
        ...
        if not inter_path:
            raise RuntimeError(
                "Cannot determine path from %s to %s" % (self.selected_file, file)
            )
    ...
    def update_binary(self, data_hex: str, offset: int = 0):
        ...
        if not isinstance(self.selected_file, TransparentEF):
            raise TypeError("Only works with TransparentEF")
        return self.rs.card._scc.update_binary(
            self.selected_file.fid, data_hex, offset, conserve=self.rs.conserve_write
        )
    ...
    def update_record(self, rec_nr: int, data_hex: str):
        ...
        if not isinstance(self.selected_file, LinFixedEF):
            raise TypeError("Only works with Linear Fixed EF")
        return self.rs.card._scc.update_record(
            self.selected_file.fid, rec_nr, data_hex, conserve=self.rs.conserve_write
        )
    ...
    def set_data(self, tag: int, data_hex: str):
        ...
        if not isinstance(self.selected_file, BerTlvEF):
            raise TypeError("Only works with BER-TLV EF")
        return self.rs.card._scc.set_data(
            self.selected_file.fid, tag, data_hex, conserve=self.rs.conserve_write
        )
...
class CardApplication:
    ...
    def __init__(
        self, name, adf: Optional[CardADF] = None, aid: str = None, sw: dict = None
    ):
        """
        Args:
            adf : ADF name
