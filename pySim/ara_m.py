...
class RefDO(
    BER_TLV_IE, tag=0xE1, nested=[AidRefDO, AidRefEmptyDO, DevAppIdRefDO, PkgRefDO]
):
    # SEID v1.1 Table 6-5
    pass
...
class ApduArDO(BER_TLV_IE, tag=0xD0):
    ...
    def _from_bytes(self, do: bytes):
        if len(do) == 1:
            ...
            if len(do) % 8:
                return ValueError(
                    "Invalid non-modulo-8 length of APDU filter: %d" % len(do)
                )
            self.decoded["apdu_filter"] = []
            offset = 0
            while offset < len(do):
                self.decoded["apdu_filter"] += {
                    "header": b2h(do[offset : offset + 4]),
        ...
        if "generic_access_rule" in self.decoded:
            ...
            for f in filters:
                ...
                if len(header_b) != 4 or len(mask_b) != 4:
                    return ValueError(
                        "APDU filter header and mask must each be 4 bytes"
                    )
                res += header_b + mask_b
            return res
...
class CommandDelete(
    BER_TLV_IE, tag=0xF1, nested=[AidRefDO, AidRefEmptyDO, RefDO, RefArDO]
):
    # SEID v1.1 Table 5-4
    pass
...
class CommandRegisterClientAidsDO(
    BER_TLV_IE, tag=0xF7, nested=[AidRefDO, AidRefEmptyDO]
):
    # SEID v1.1 Table 5-7
    pass
...
class StoreCommandDoCollection(
    TLV_IE_Collection,
    nested=[
        BlockDO,
        CommandStoreRefArDO,
        ...
        CommandGet,
        CommandGetAll,
        CommandGetClientAidsDO,
        CommandGetNext,
        CommandGetDeviceConfigDO,
    ],
):
    pass
...
class StoreResponseDoCollection(
    TLV_IE_Collection,
...
class ADF_ARAM(CardADF):
    ...
    class AddlShellCommands(CommandSet):
        ...
        store_ref_ar_do_parse.add_argument(
            "--device-app-id",
            required=True,
            help="Identifies the specific device application that the rule appplies to. Hash of Certificate of Application Provider, or UUID. (20/32 hex bytes)",
        )
        aid_grp = store_ref_ar_do_parse.add_mutually_exclusive_group()
        aid_grp.add_argument(
            "--aid",
            help="Identifies the specific SE application for which rules are to be stored. Can be a partial AID, containing for example only the RID.  (5-16 hex bytes)",
        )
        aid_grp.add_argument(
            "--aid-empty",
            action="store_true",
        ...
        apdu_grp.add_argument(
            "--apdu-filter",
            help="APDU filter: 4 byte CLA/INS/P1/P2 followed by 4 byte mask (8 hex bytes)",
        )
        nfc_grp = store_ref_ar_do_parse.add_mutually_exclusive_group()
        nfc_grp.add_argument(
            "--nfc-always", action="store_true", help="NFC event access is allowed"
        )
        nfc_grp.add_argument(
            "--nfc-never", action="store_true", help="NFC event access is not allowed"
        )
        store_ref_ar_do_parse.add_argument(
            "--android-permissions",
            help="Android UICC Carrier Privilege Permissions (8 hex bytes)",
        )
        ...
        def do_aram_store_ref_ar_do(self, opts):
            ...
            if opts.android_permissions:
                ar_do_content += [
                    {"PermArDO": {"permissions": opts.android_permissions}}
                ]
